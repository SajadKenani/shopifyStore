...
const navigate = useNavigate()
    ...
    localStorage.setItem("thedescription", myArray[6].toString()); 
    }
    ...
    const description = JSON.stringify(localStorage.getItem("thedescription")).split(`"`)[1];
    const indescriptioned = description.substring(0, description.length);
...
return (
    <div className="myOuterDiv-div">
     <div className="tweakingDiv-div">
        ...
        {myStock === 0 && isPurchesed === false ? <button className="myUnAvailableButton">Not Available</button> :
        isPurchesed === false &&
        ...
        {/* The Description*/}
        <p className="description-para">{indescriptioned}</p>   
        </div>
        </div>
    </div>
</div>
