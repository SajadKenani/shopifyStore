...
<Container className="" style={{ marginTop: 10+"px"}}>
 ...
 .filter( data => 
  ...
  myPrice ? (Number(data.price) > myFirstValue) && (Number(data.price) < mySecondValue) :(Number(data.price) > 0) && 
  (Number(data.price) < 10000) &&
  // To display the cards according to their period 
  myPeriod ? (Number(data.period) > firstPeriodValue) && (Number(data.period) < secondPeriodValue) :(Number(data.period) > 0) && 
  (Number(data.period) < 10000) 
  )
     ...
     {<div className="card " key={"divTwo "+b}>
      ...
      <Link to="/card" style={{textDecoration: "none", color: "black"}}>
       {/* To Send Info to To Card Page */}
        <div className="card-body text-left" key={"divThree "+b} onClick={() => 
          getInfo([a.name, a.price, a.image, a.period, a.inStock, b, a.description])}>   
        {/* Displaying the images...   */}
          ...
          <p key={"PThree "+b} className="text-secondary">{Number(a.period) >= 12 && <p style={{display:"flex"}}>
            Released {(Number(a.period) * 0.0833334).toFixed()}&nbsp;{Number((Number(a.period) * 0.0833334).toFixed()) !== 1 ? 
            <p> years ago </p> : <p> year ago </p>} </p>}
          <p key={"PFour "+b} className="text-secondary">{Number(a.period) < 12 && Number(a.period) > 0.9 && 

          <p style={{display:"flex"}}> Released {(Number(a.period)).toFixed()}&nbsp;{(Number(a.period)) !== 1 ? 
          <p>months ago</p> : <p>month ago</p>} </p>}
